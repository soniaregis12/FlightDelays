...
public class ExtFlightDelaysDAO {
	...
	public int getAirlinesNumber(Airport a) {
		...
		String sql = "SELECT COUNT(DISTINCT(AIRLINE_ID)) AS tot FROM flights AS f WHERE (f.ORIGIN_AIRPORT_ID =?) || (f.DESTINATION_AIRPORT_ID =?)";
		int res = -1;
	...
	public List<Rotta> getRotte(Map<Integer, Airport> idMap) {
		String sql = "SELECT f.ORIGIN_AIRPORT_ID, f.DESTINATION_AIRPORT_ID, COUNT(*) AS tot FROM flights AS f GROUP BY f.ORIGIN_AIRPORT_ID, f.DESTINATION_AIRPORT_ID";
		...
		try {
			...
			while(rs.next()) {
				...
				if(sorgente != null && destinazione != null){
					rotte.add(new Rotta(sorgente, destinazione, rs.getInt("tot")));
				}else {
